...
public abstract class DBHelperBase<E, A> extends SQLiteOpenHelper {
	...
	public DBHelperBase(Context context, String user, int version,
			String table, String id, Class<?> c) {
		super(context, user + "_" + table, null, version);
		this.mUser = user;
		this.mClass = c;
		...
		try {
			...
			if (Modifier.isPrivate(fieldKey.getModifiers())) {
				if (fieldKey.getType() == String.class)
					...
					TABLE_SQL = TABLE_SQL + " INTEGER PRIMARY KEY ";
	...
	protected final boolean insertInTransaction(String table,
	...
	protected final boolean updateInTransaction(String table,
	...
	protected final boolean deleteInTransaction(String table, String where,
			String[] whereArgs) {
	...
	public ContentValues getContentValuesFor(E object, boolean b) {
				...
				if (Modifier.isPrivate(field.getModifiers())) {
					if (field.getType() == String.class) {
						if (field.get(object) != null)
							contentValues.put(field.getName(), field
									.get(object).toString());
						else
							contentValues.put(field.getName(), "");
					} else if (field.getType().toString().equals("int")) {
						contentValues
								.put(field.getName(), field.getInt(object));
		...
		if (b) {
			...
			contentValues.put(MODIFIED_DATE,
					DateTimeHelper.toString(System.currentTimeMillis()));
		}
			...
			for (Field field : object.getClass().getDeclaredFields()) {
				...
				if (Modifier.isPrivate(field.getModifiers())) {
					if (field.getType() == String.class)
						...
						field.setInt(object, cursor.getInt(cursor
								.getColumnIndex(field.getName())));
					else if (field.getType().toString().equals("long"))
						field.setLong(object, cursor.getLong(cursor
								.getColumnIndex(field.getName())));
					else if (field.getType().toString().equals("double"))
						field.setDouble(object, cursor.getDouble(cursor
								.getColumnIndex(field.getName())));
				}
			}
		} catch (IllegalArgumentException e) {
	...
	public abstract boolean insertWS(E object);

	protected boolean insert(String table, E object) {
		boolean wasInserted = false;
		openDB();
		wasInserted = insertInTransaction(table,
				getContentValuesFor(object, true));
		closeDB();
		return wasInserted;
	}

	protected boolean insertWS(String table, E object) {
		boolean wasInserted = false;
		openDB();
		wasInserted = insertInTransaction(table,
				getContentValuesFor(object, false));
		closeDB();
		return wasInserted;
	}
	...
	protected boolean delete(String table, String keyName, A key) {
		boolean wasDeleted = false;
		openDB();
		wasDeleted = deleteInTransaction(table, keyName + "=?",
	...
	public abstract boolean updateWS(A key, E object);

	protected boolean update(String table, String keyName, A key, E object) {
		boolean wasUpadated = false;
		openDB();
		wasUpadated = updateInTransaction(table,
				getContentValuesFor(object, true), keyName + "=?",
				new String[] { key.toString() });
		closeDB();
		return wasUpadated;
	}

	protected boolean updateWS(String table, String keyName, A key, E object) {
		boolean wasUpadated = false;
		openDB();
		wasUpadated = updateInTransaction(table,
				getContentValuesFor(object, false), keyName + "=?",
				new String[] { key.toString() });
		closeDB();
		return wasUpadated;
	}
	...
	protected E select(String table, String keyName, A key) {
		E object = null;
		openDB();
		Cursor cursor = mDb.query(table, null, keyName + "=?",
	...
	protected ArrayList<E> selectAll(String table, String key, String value) {
		ArrayList<E> objects = null;
		openDB();
		Cursor cursor = mDb.query(table, null, key + "=?",
	...
	protected ArrayList<E> selectAllLike(String table, String key,
			String value, String sort) {
		ArrayList<E> objects = null;
		openDB();
		Cursor cursor = mDb.query(table, null, key + "=?" + " AND " + sort,
				new String[] { value }, null, null, null);
		objects = new ArrayList<E>();
		while (cursor.moveToNext())
			objects.add(getObjectFromCursor(cursor));
	...
	// modified date
	public abstract boolean setModifiedDate(A key, String date);

	protected boolean setModifiedDate(String table, String keyName, A key,
			String date) {
		boolean wasUpdated = false;
		openDB();
		ContentValues contentValues = new ContentValues();
		wasUpdated = mDb.update(table, contentValues, keyName + "=?",
				new String[] { key.toString() }) > 0;
		closeDB();
		return wasUpdated;
	}

	public abstract String getModifiedDate(A key);

	protected String getModifiedDate(String table, String keyName, A key) {
		String date = null;
		openDB();
		Cursor cursor = mDb.query(table, new String[] { MODIFIED_DATE },
				keyName + "=?", new String[] { key.toString() }, null, null,
				null);
		if (cursor.moveToNext())
	...
	protected boolean exists(String table, String keyName, A key) {
		...
		openDB();
		Cursor cursor = mDb.query(table, new String[] { keyName }, keyName
				+ "=?", new String[] { key.toString() }, null, null, null);
